...
from backend.utils.inference.rcs_generators.rcs_generator_engine import build_archetype_subgraph_with_temporal_depth, build_persona_adjacency_from_subgraph, generate_rcs
...
def simulate_rcs(product_subgraph, archetype_id, zmot_id = None):
    ...
    for pain_trigger_id in archetype_pain_trigger_ids:    
        ...
        baseline_pain_trigger_likelihood = get_edge_attribute(archetype_subgraph, pain_trigger_id, archetype_id, "likelihood")
        ...
        for pain_id in pain_ids:
            ...
            pain_likelihood = get_edge_attribute(archetype_subgraph, pain_id, pain_trigger_id, "likelihood") * pain_trigger_likelihood
            ...
            for perceived_metric_id in perceived_metric_ids:
                ...
                metric_likelihood = get_edge_attribute(archetype_subgraph, pain_id, perceived_metric_id, "likelihood") * pain_likelihood
                metric = {
            ...
            solving_node_ids = get_source_nodes_by_target_and_type(archetype_subgraph, pain_id, "solves")
            for solving_node_id in solving_node_ids:
                solving_node = get_node_by_id(archetype_subgraph, solving_node_id)
                
                if not solving_node:
                    print("Solving node is not found")
                    continue


                if solving_node.get("type") != "job":
                    solving_capability_id = solving_node_id
                    solving_capability = get_node_by_id(archetype_subgraph, solving_capability_id)
                    capability_likelihood = get_edge_attribute(archetype_subgraph, solving_capability_id, pain_id, "likelihood")* pain_likelihood
                    
                    solving_capability = {
                        "capability_id": solving_capability_id,
                        "capability_name": solving_capability.get("name", "UNKNOWN"),
                        "capability_description": solving_capability.get("description", "UNKNOWN"),
                        "likelihood": capability_likelihood,
                        "depth": solving_capability.get("depth", 0)
                    }
                    solving_capabilities.append(solving_capability)

                    continue
                if solving_node.get("type") == "job":
                    solving_job_id = solving_node_id
                    solving_job_node = solving_node

                    solving_job_depth = solving_job_node.get("depth")
                    solving_job_description = solving_job_node.get("description", "Unknown")
                    solving_job_likelihood = get_edge_attribute(archetype_subgraph, solving_job_id, pain_id, "likelihood") * pain_likelihood
                    ...
                    for persona_job_id in persona_job_ids:
                        ...
                        persona_job_likelihood = get_edge_attribute(archetype_subgraph, solving_job_id, persona_job_id, "likelihood") * solving_job_likelihood
            ...
            # Logic to ensure only pains with solving jobs are included
            if len(solving_jobs) > 0:
                pain_set = {
                    "pain_id": pain_id,
                    ...
                    #"solving_capabilities": solving_capabilities
                }
                pain_sets.append(pain_set)
        # Logic to ensure only pain triggers with valid pain_sets are included
        if len(pain_sets) > 0:
            pain_trigger_res = {
    ...
    #most_likely_capability = max(most_likely_pain.get("solving_capabilities", []), key=lambda x: x["likelihood"], default=None) if most_likely_pain else None
    #print("Most likely capability family:", most_likely_capability)
    ...
    for path in causal_paths:
        ...
        while path:
            ...
            if node.get("type") == "job":
                persona_ids = get_target_nodes_by_source_and_type(archetype_subgraph, node_id, "performed_by")
                if persona_ids:
                    # Pick the persona with the highest likelihood
                    persona_likelihoods = [
                        (pid, get_edge_attribute(archetype_subgraph, node_id, pid, "likelihood"))
                        for pid in persona_ids
                    ]
                    persona_id, _ = max(persona_likelihoods, key=lambda x: x[1])
                    node_id = persona_id
            ...
            path_likelihood *= get_edge_attribute(archetype_subgraph, path[0], node_id, "likelihood") if path else 1
            causal_chain.append({
    ...
    # Sort personas within each job
    for trigger in first_response_pain_family:
        for pain in trigger.get("pains", []):
            for job in pain.get("solving_jobs", []):
                job["persona_jobs"] = sorted(job.get("persona_jobs", []), key=lambda x: x["likelihood"], reverse=True)
            # Sort jobs within each pain
            pain["solving_jobs"] = sorted(pain.get("solving_jobs", []), key=lambda x: x["likelihood"], reverse=True)
        # Sort pains within each trigger
        trigger["pains"] = sorted(trigger.get("pains", []), key=lambda x: x["likelihood"], reverse=True)

    # Sort triggers at the top level
