...
def is_sparse_results(relevance_results):
    persona_job_map = {}
    job_pain_map = {}
    cap_pain_map = {}
    ...
    for entry in relevance_results:
        ...
        # Use canonical persona
        persona_key = PERSONA_CANONICAL_MAP.get(persona_key, persona_key)
    ...
    print("Persona threshold:", persona_threshold)
    print("Job threshold:", job_threshold)
    print("Capability threshold:", capability_threshold)
    print("Sparse results detected:", persona_threshold or job_threshold or capability_threshold)
    print("Total personas:", len(persona_job_map))

    return persona_threshold or job_threshold or capability_threshold


# 🧠 Rule-based first, fallback to 2-step OpenAI reasoning
def infer_with_rules_then_fallback(summary, capabilities, force_openai=False, retry_depth=0, cached_relevance=None) -> dict:
    ...
    print("Loaded JOB_CANONICAL_MAP with", len(JOB_CANONICAL_MAP), "entries")
    print("Loaded PAIN_CANONICAL_MAP with", len(PAIN_CANONICAL_MAP), "entries")

    try:
        # Step 1: Match capabilities to canonical pains, jobs, and personas
        print("🧠 Trying rule-based relevance scoring...")
        relevance_results = match_capabilities_to_canonical_personas(capabilities)

        if relevance_results and len(relevance_results) > 0:
            print("Relevance results found. Processing...")
            sparse = is_sparse_results(relevance_results)
            ...
            print("Not sparse - building capability map")
            capability_map = convert_rule_matches_to_capability_map(relevance_results)
            flattened_results = process_capability_map_to_graph(capability_map)

            # Merge relevance scores into flattened_results
            for result in flattened_results:
                for relevance in relevance_results:
                    if (
                        result["job"] == relevance["canonical_job"]
                        and result["pain"] == relevance["canonical_pain"]
                    ):
                        result["relevance"] = relevance["relevance"]
                        break
                else:
                    # Assign high relevance for OpenAI-generated outputs
                ...
                for pain_entry in capability_entry.get("pains", []):
                    ...
                    for job_entry in pain_entry.get("jobs", []):
                        ...
                        for persona in job_entry.get("personas", []):
                            capability_map[capability][pain][job].append(persona)
    
            
        print("📊 [Graph] Flattening capability map...")
        flattened_capability_map = []
        for capability, pains in capability_map.items():
            for pain, jobs in pains.items():
                for job, personas in jobs.items():
